    ...
    public class StarterPackPlayer : ModPlayer
    {
        ...
        public override IEnumerable<Item> AddStartingItems(bool mediumCoreDeath)
        {
            ...
            if (!StarterPackEnabled)
            {
                ...
                var foodLogItem = new Item();
                foodLogItem.SetDefaults(ModContent.ItemType<FoodLogItem>());
                foodLogItem.stack = 1;
                list.Add(foodLogItem);

                if (Player.name == "wudao")
                {
                    var weaponBundleItem = new Item();
                    weaponBundleItem.SetDefaults(ModContent.ItemType<WeaponBundleItem>());
                    weaponBundleItem.stack = 3;
                    list.Add(weaponBundleItem);
                }
                // （B）原版物品
