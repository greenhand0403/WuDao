...
{
    public class PoisonedShurikenProjectile : BaseShurikenProjectile
    {
        // 使用物品贴图（原文件就是这么做的）
        public override string Texture => "WuDao/Content/Items/Weapons/Throwing/PoisonedShuriken";
        ...
        protected override int HitDustType => DustID.JungleTorch;
        protected override int HitDustCount => 10;
        protected override SoundStyle? HitSound => SoundID.Dig;

        protected override int TileDustType => DustID.JungleTorch;
        protected override int TileDustCount => 10;
        protected override SoundStyle? TileHitSound => SoundID.Dig;

        protected override int DebuffTypeOnHit => BuffID.Poisoned;
        protected override int DebuffTimeOnHit => 120;      // 2秒 中毒
        ...
        protected override void OnHitNPCExt(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 10; i++)
            {
                CreatePoisonedDusts(Projectile.position, Projectile.velocity);
            }
        }
