...
namespace WuDao.Content.Projectiles.Throwing
{
    ...
    {
        // 使用物品贴图（原文件就是这么做的）
        public override string Texture => "WuDao/Content/Items/Weapons/Throwing/FlameShuriken";
        ...
        protected override void OnHitNPCExt(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 10; i++)
            {
                CreateDusts(Projectile.position, Projectile.velocity);
            }
        }
        ...
        private void CreateDusts(Vector2 pos, Vector2 vel)
        {
