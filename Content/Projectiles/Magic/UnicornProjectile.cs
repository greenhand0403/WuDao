...
namespace WuDao.Content.Projectiles.Magic
{
    ...
    {
        public override string Texture => $"Terraria/Images/NPC_{NPCID.Unicorn}";
        public int FrameTicksPerFrame = 5;
        public float MinSpeed = 10f;
        public override void SetStaticDefaults()
        {
            // 读取帧数，避免魔法数字
            Main.projFrames[Projectile.type] = Main.npcFrameCount[NPCID.Unicorn];
        }
        public override void SetDefaults()
        {
            Projectile.width = 100;  // 根据独角兽贴图大致调整
        ...
        public override void AI()
        {
            ...
            if (++Projectile.frameCounter >= FrameTicksPerFrame)
            {
                ...
                if (Projectile.frame >= Main.projFrames[Projectile.type]-1)
                    Projectile.frame = 10;//这一帧 开始飞奔动画
        ...
        {
            // 使用目标 NPC 的贴图 + 帧
            Texture2D tex = TextureAssets.Npc[NPCID.Unicorn].Value;
