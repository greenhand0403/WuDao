...
namespace WuDao.Content.Projectiles.Melee
{
    public class GlitteringClawHoldout : ModProjectile
    {
        ...
        private const float RecoilSpeed = 15.2f;        // 命中后玩家轻微后坐
        private const int LocalIFrames = 10;            // 对同一NPC的本地无敌帧
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            float scale = target.boss ? 1f : 0.25f;

            player.velocity -= dir * RecoilSpeed * player.GetTotalArmorPenetration(DamageClass.Melee) * scale;
